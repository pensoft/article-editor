...
const replaceStep = (
    ...
    const positionTo = deletionMark && deletionMark.contained ? deletionMark.to : step.to;

    let deleteMarkId = deletionMark ? deletionMark.attrs.id : uuidv4()

    let insideDeleteMark = deletionMark ? deletionMark.contained : false;
    let atednOfDeleteMark = deletionMark && deletionMark.atStartOfSel ? true : false;
    let atStartOfDeletionMark = deletionMark && deletionMark.atEndOfSel ? true : false;
    ...
    if (newStep) {
        ...
        if (insideDeleteMark && state.selection.$to.nodeAfter) {
            let resolvedPosAtEndOfContainedDeletion = state.doc.resolve(state.selection.to + state.selection.$to.nodeAfter.nodeSize);
            if (resolvedPosAtEndOfContainedDeletion.nodeAfter) {
                let insertionMarkAtEndOfDeletionMark = resolvedPosAtEndOfContainedDeletion.nodeAfter.marks.filter(mark => mark.type.name == insertionMarkFromSchema.name)[0]

                if (insertionMarkAtEndOfDeletionMark) {
                    insertionMarkAttrs = insertionMarkAtEndOfDeletionMark.attrs;
                    userHasInsertionMarkAtEndOdDeletionMark = insertionMarkAtEndOfDeletionMark.attrs.user == user.userId;
                }
            }
        } else if (atednOfDeleteMark && state.selection.$to.nodeAfter) {
            let insertionMarkAtEndOfDeletionMark = state.selection.$to.nodeAfter.marks.filter(mark => mark.type.name == insertionMarkFromSchema.name)[0]
            if (insertionMarkAtEndOfDeletionMark) {
                insertionMarkAttrs = insertionMarkAtEndOfDeletionMark.attrs;
                userHasInsertionMarkAtEndOdDeletionMark = insertionMarkAtEndOfDeletionMark.attrs.user == user.userId;
            }
        } else if (atStartOfDeletionMark && state.selection.$from.nodeBefore) {
            let insertionMarkAtStartOfDeletionMark = state.selection.$from.nodeBefore.marks.filter(mark => mark.type.name == insertionMarkFromSchema.name)[0]
            if (insertionMarkAtStartOfDeletionMark) {
                insertionMarkAttrs = insertionMarkAtStartOfDeletionMark.attrs;
                userHasInsertionMarkAtStartOFDeletionMark = insertionMarkAtStartOfDeletionMark.attrs.user == user.userId;
            }
        }

        let userIsContinuingItsOwnMarkAroundADeletion = userHasInsertionMarkAtEndOdDeletionMark || userHasInsertionMarkAtStartOFDeletionMark
        ...
        if (thereIsADeletionAroundcursur) {
            if (userIsContinuingItsOwnMarkAroundADeletion) {
                markInsertion(trTemp, newStep.from, mappedNewStepTo, user, date, group, insertionMarkID, deleteMarkId, insertionMarkAttrs);
            } else {
                insertionMarkID = uuidv4()
                markInsertion(trTemp, newStep.from, mappedNewStepTo, user, date, group, insertionMarkID, deleteMarkId, undefined);
            }
        } else {
            let insMarkBefore
            let insMarkAfter
            let nodeBefore = state.selection.$from.nodeBefore || state.selection.$to.nodeBefore
            let nodeAfter = state.selection.$from.nodeAfter || state.selection.$to.nodeAfter
            if (nodeBefore) {
                insMarkBefore = nodeBefore.marks.filter(mark => mark.type.name == insertionMarkFromSchema.name)[0]
            }
            if (nodeAfter) {
                insMarkAfter = nodeAfter.marks.filter(mark => mark.type.name == insertionMarkFromSchema.name)[0]
            }
            if (insMarkBefore && insMarkBefore.attrs.id == lastContainingInsertionMark) {
                insertionMarkAttrs = insMarkBefore.attrs
            }
            if (insMarkAfter && insMarkAfter.attrs.id == lastContainingInsertionMark) {
    ...
    if (step.from !== step.to) {
        map.appendMap(markDeletion(newTr, step.from, step.to, user, date, group, deleteMarkId, insertionMarkID ? insertionMarkID : '', deletionMark ? deletionMark.attrs : undefined));
    }
};
