...
export function getElementWithLargestNumberInCitationWithNestedElement(
  ...
  displayedType:'figure'|'table',
    ...
    let allNestedCitedElements = []
    citedElementsObj;

    displayedElementeViews.forEach((viewId)=>{
      getAllCitedElements(citatType,viewId,figuresData,tablesData,figuresOrderArr,tablesOrderArr,citedElementsObj);
    })
    console.log(citedElementsObj,'figuresOrderArr',figuresOrderArr,'tablesOrderArr',tablesOrderArr);
...
export function getAllCitedElements(
  ...
  citedElsObj:{tables:string[],figures:string[]}){
  ...
  if(citationType == 'citation'){
    ...
    if(figureData){
      ...
      //console.log(htmlStringsToCheck);
    }
  ...
  //console.log(htmlStringsToCheck,'citedTablesIds',citedTablesIds,'citedFiguresIds',citedFiguresIds);
  ...
  if(newTableIds.length>0){
    newTableIds.forEach((tblId)=>{
      getAllCitedElements('table_citation',tblId,figuresData,tablesData,figuresNumbers,tablesNumbers,citedElsObj);
    })
  }
  if(newFigureIds.length>0){
    newFigureIds.forEach((figId)=>{
      getAllCitedElements('citation',figId,figuresData,tablesData,figuresNumbers,tablesNumbers,citedElsObj);
    })
  }
}
